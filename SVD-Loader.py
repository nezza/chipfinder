...
svd_file = sys.argv[1]
chipfinder_file = os.path.basename(svd_file) + ".chipfinder"
...
with open(chipfinder_file, "w") as csvfile:
	w = csv.writer(csvfile, delimiter=',',
                            quotechar='"', quoting=csv.QUOTE_MINIMAL)
	for peripheral in peripherals:
		print(peripheral.name)
		print(peripheral.base_address)
		try:
			for register in peripheral.registers:
				print("\t%s" % register.name)
				print("\t%d" % register.address_offset)
				w.writerow([peripheral.name + " " + register.name, peripheral.base_address + register.address_offset])
		except:
			continue
